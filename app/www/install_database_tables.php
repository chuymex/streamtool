...
if (isset($_GET['install'])) {
    ...
    // Obtener tablas existentes según el driver
    if ($capsule->getConnection()->getDriverName() === 'sqlite') {
        // En SQLite usamos sqlite_master
        $tables = $capsule::select("SELECT name FROM sqlite_master WHERE type='table'");
        foreach ($tables as $val) {
            $tableName = (array) $val;
            $tableName = array_shift($tableName);
            $arraynamesexist[] = $tableName;
        }
    } else {
        // Comportamiento original para MySQL
